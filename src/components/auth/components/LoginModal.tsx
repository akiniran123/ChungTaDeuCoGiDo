        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="flex w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden bg-white dark:bg-gray-900">
            ...
            <div className="w-full md:w-1/2 p-10 flex flex-col justify-center relative">
              <button onClick={onClose} className="absolute top-5 right-5 w-12 h-12 text-4xl text-gray-500 hover:text-gray-900 dark:hover:text-white cursor-pointer">
                ×
              </button>

              <h2 className="text-3xl font-bold text-center mb-8 text-gray-900 dark:text-white">Chào mừng trở lại</h2>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-indigo-600 text-white py-3 rounded-xl font-semibold hover:bg-indigo-700 transition-all active:scale-[0.97] disabled:opacity-60 cursor-pointer"
                >
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full flex items-center justify-center gap-3 py-3 rounded-xl border border-gray-300 hover:bg-gray-50 transition-all active:scale-[0.97] disabled:opacity-60 cursor-pointer"
                >
                  <FcGoogle className={`w-6 h-6 ${loading ? "animate-spin" : ""}`} />
                  {loading ? "Đang chuyển hướng..." : "Tiếp tục với Google"}
                </button>

                <div className="flex justify-between text-sm mt-4">
                  <button type="button" onClick={() => setActiveModal("signup")} className="text-indigo-600 hover:underline cursor-pointer">
                    Đăng ký
                  </button>
